...
const ServiceProviderCard: React.FC<{ provider: UserEntity; onHire: (provider: UserEntity) => void; }> = ({ provider, onHire }) => {
    return (
        <div className={`bg-slate-900/50 p-3 rounded-xl border flex space-x-3 ${provider.isFounder ? 'border-pi-gold/40' : 'border-white/10'}`}>
            <div className="relative">
                <img src={provider.avatarUrl} alt={provider.piUsername} className="w-16 h-16 rounded-full border-2 border-slate-600" />
                {provider.isFounder && (
                     <div className="absolute -bottom-1 -right-1 bg-pi-gold text-brand-dark text-[8px] font-bold px-1.5 py-0.5 rounded-full border border-brand-dark">
                        FOUNDER
                    </div>
                )}
            </div>
            <div className="flex-grow">
                <div className="flex items-center">
                     <h5 className="font-bold text-white mr-1">{provider.piUsername}</h5>
                     {provider.isFounder && <ShieldCheckIcon className="w-3 h-3 text-pi-gold" title="Founder Member" />}
                </div>
